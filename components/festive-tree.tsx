...
export function FestiveTree() {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            const pContainer = mainSVG.querySelector('.pContainer') as SVGGElement
            const treePath = mainSVG.querySelector('.treePath') as SVGPathElement
            const treeBottomPath = mainSVG.querySelector('.treeBottomPath') as SVGPathElement
            const sparkle = mainSVG.querySelector('.sparkle') as SVGCircleElement

            // Helper to get path points (simplified version of what the original script did)
            // We'll just use MotionPathPlugin directly instead of manual point extraction for simplicity where possible
            ...
            for (let i = 0; i < 200; i++) {
                ...
                const template = mainSVG.querySelector(shapeId) as SVGElement
                if (template) {
            ...
            function spawnParticle() {
                ...
                // Get current position of the "emitter" (pContainer/sparkle which are moving along path)
                // Note: In the original, pContainer itself was moving. 
                // We need to get the current coordinates of the sparkle/emitter.
                ...
                // Simulate Physics2D with standard tweens
                const angle = gsap.utils.random(-180, 180) * (Math.PI / 180)
                const velocity = gsap.utils.random(20, 100)
                const duration = gsap.utils.random(0.6, 2)

                const destX = (x as number) + Math.cos(angle) * velocity
                const destY = (y as number) + Math.sin(angle) * velocity + (gsap.utils.random(50, 200)) // Gravity effect
            ...
            // Move container along tree path
            // Note: MotionPathPlugin requires the path element or data
            tl.to(pContainer, {
                duration: 6,
                motionPath: {
                    path: ".treePath",
                ...
                // Jump to bottom path start
                .set(pContainer, {
                    x: 207.5, // Approximate start of treeBottomPath
                    y: 484.1
                })
                // Move along bottom path
                .to(pContainer, {
                    duration: 2,
                    motionPath: {
                        path: ".treeBottomPath",
            ...
            // Sparkle follows the same movement (it's inside pContainer or moving with it)
            // In the original, sparkle and pContainer were animated together.
            // Here we just animate pContainer and let sparkle sit at 0,0 inside it? 
            // Actually sparkle is a sibling in the original HTML structure but animated together.
            // Let's just animate the sparkle separately if needed or assume it tracks.
            // For simplicity, let's just animate the sparkle to follow the container visually.

            // DrawSVG replacements
            // We need to set stroke-dasharray/offset for the masks
            const masks = [".treePathMask path", ".treePotMask path", ".treeBottomMask path"]
            masks.forEach(selector => {
                const el = mainSVG.querySelector(selector) as SVGPathElement
                if (el) {
            ...
            // Animate the drawing of the tree
            const drawTl = gsap.timeline()

            // Tree Body
            drawTl.to(".treePathMask path", {
                strokeDashoffset: 0,
                duration: 6,
                ease: "linear"
            })
                // Tree Bottom
                .to(".treeBottomMask path", {
                    strokeDashoffset: 0,
                    duration: 2,
                    ease: "linear"
                })
            ...
            gsap.from(".treeStar", {
                ...
                delay: 6 // Wait for tree to draw mostly
            })

            // Sparkle fade out at end
            gsap.to(sparkle, {
                duration: 3,
                opacity: 0,
                repeat: -1,
    ...
    return (
        <div ref={containerRef} className="w-full h-full flex items-center justify-center bg-[#151522] overflow-hidden">
            <svg
                ref={svgRef}
                className="mainSVG w-full h-full max-w-[800px] max-h-[600px]"
                viewBox="0 0 800 600"
            ...
            >
                <defs>
                    ...
                    <path id="heart" className="particle" d="M2.9,0C2.53,0,2.2,0.18,2,0.47C1.8,0.18,1.47,0,1.1,0C0.49,0,0,0.49,0,1.1 C0,2.6,1.56,4,2,4s2-1.4,2-2.9C4,0.49,3.51,0,2.9,0z" />
                    ...
                    <mask id="treePathMask">
                        <path className="treePathMask" fill="none" strokeWidth="18" stroke="#FFF" d="M252.9,447.9c0,0-30.8-21.6,33.9-44.7c64.7-23.1,46.2-37,33.9-41.6 c-12.3-4.6-59.3-11.6-42.4-28.5s114-52.4,81.7-66.2c-32.4-13.9-58.5-10.8-35.4-29.3s66.2-101.7,70.9-115.6 c4.4-13.2,16.9-18.5,24.7,0c7.7,18.5,44.7,100.1,67.8,115.6c23.1,15.4-10.8,21.6-26.2,24.7c-15.4,3.1-20,33.9,33.9,49.3 c53.9,15.4,47.8,40.1,27.7,44.7c-20,4.6-63.2,4.6-27.7,32.4s98.6,21.6,61.6,60.1" />
                    </mask>
                ...
                <g className="whole">
                    ...
                    <g className="treeBottom" mask="url(#treeBottomMask)">
                        <path d="M207.5,484.06c7.05-5.11,15.14-8.66,23.34-11.63a177.13,177.13,0,0,1,25.29-6.88,263.65,263.65,0,0,1,52.22-4.49h3.28l3.28.09,6.56.19,6.55.39c2.18.13,4.37.26,6.54.48,4.35.39,8.71.74,13,1.28l6.51.75,6.49.91c17.3,2.5,34.41,6,51.36,10.19l12.62,3.2c4.18,1,8.34,2.18,12.55,3.06,8.38,2,16.82,3.63,25.29,5.13a353.5,353.5,0,0,0,51.17,5.47c17.11.32,34.36-.66,51-4.7a118.55,118.55,0,0,0,24.21-8.47,84.82,84.82,0,0,0,11.11-6.49,47.55,47.55,0,0,0,9.69-8.53,48.1,48.1,0,0,1-9,9.45,85.1,85.1,0,0,1-10.81,7.45,116.56,116.56,0,0,1-24.23,10.24,165.66,165.66,0,0,1-25.79,5.35,232.1,232.1,0,0,1-26.27,1.71c-8.77,0-17.55-.24-26.26-1.09-2.18-.2-4.37-.35-6.54-.6l-6.52-.78c-4.36-.46-8.67-1.19-13-1.82-8.64-1.37-17.22-3.09-25.74-5-4.28-.87-8.5-2-12.75-3l-12.62-3.11q-25.06-6.37-50.58-10.47a426.37,426.37,0,0,0-51.3-5.3c-8.59-.42-17.19-.29-25.78,0a240.1,240.1,0,0,0-25.68,2.24,186.57,186.57,0,0,0-25.27,5.19c-4.15,1.16-8.26,2.49-12.28,4.05-2,.79-4,1.6-6,2.52A50.82,50.82,0,0,0,207.5,484.06Z" fill="#cb9866" />
                    </g>

                    <g className="treePot" mask="url(#treePotMask)">
                        <path d="M374.32,502.55a48.15,48.15,0,0,0,1.24,14.35c1.15,4.52,3.29,8.64,6.49,11.35a18.5,18.5,0,0,0,5.51,3.14,39.06,39.06,0,0,0,6.41,1.82,65.78,65.78,0,0,0,13.68,1.12,72.9,72.9,0,0,0,13.72-1.44,44.51,44.51,0,0,0,6.46-1.85,17.75,17.75,0,0,0,5.51-3.15,25.45,25.45,0,0,0,7.24-11.17,52,52,0,0,0,1.9-6.91c.48-2.37.83-4.8,1.18-7.25a55.16,55.16,0,0,1,.64,7.42,40.11,40.11,0,0,1-.52,7.56,31.23,31.23,0,0,1-2.19,7.5,24.37,24.37,0,0,1-4.46,6.79,25.16,25.16,0,0,1-6.61,5,39.72,39.72,0,0,1-7.4,3A58.55,58.55,0,0,1,407.75,542a55,55,0,0,1-15.47-1.9,36.65,36.65,0,0,1-7.46-3,25.3,25.3,0,0,1-6.6-5,19.63,19.63,0,0,1-2.5-3.34,21.72,21.72,0,0,1-1.79-3.67,27.66,27.66,0,0,1-1.65-7.7,38.16,38.16,0,0,1,2-14.87Z" fill="#cb9866" />
                    </g>
